...
public class JuanSpawnCommand implements CommandExecutor {
    ...
    /**
     * This command will spawn the Horse
     */
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (p.hasPermission("juan.first.juan")) {
                if (plugin.vaultEconomy().has(p, plugin.getConfig().getDouble("First-Horse-Cost"))) {
                    plugin.vaultEconomy().withdrawPlayer(p, plugin.getConfig().getDouble("First-Horse-Cost"));
                    String Money = plugin.ConfigMessage.getString("OnCommandPaymentMessage");
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', Money + ""));
                    ...
                    if (plugin.getConfig().getBoolean("First-Horse-NameVisible")) {
                        horse.setCustomName(plugin.ut.chatcolor(plugin.getConfig().getString("First-Horse-Name"))
                                .replace("%first-horse-name%", p.getName()));
                    ...
                    String NoMoney = plugin.ConfigMessage.getString("NotEnoughMoney");
                    p.sendMessage(ChatColor.translateAlternateColorCodes('&', NoMoney + ""));
                    return true;
                }
            } else {
